...
def sublist(list_one, list_two):
    ...
    str_list_one = "".join([str(number) for number in list_one])
    str_list_two = "".join([str(number) for number in list_two])
    
    if str_list_one in str_list_two:
        if len(list_one) > len(list_two):
            return UNEQUAL
        if len(list_one) == len(list_two) and len(str_list_one) != len(str_list_two):
            return UNEQUAL
        return SUBLIST
