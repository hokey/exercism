...
for size in range(1,10):
    for combination in itertools.combinations(SUDOKU_DIGITS, size):
        PRECOMPUTED[(size, sum(combination))].append(combination)
...
def _filtered_combinations(size: int, target: int, exclude: tuple[int, ...]) -> tuple[tuple[int, ...], ...]:
    ...
    return tuple(
        ...
        if not any(digit in exclude_set for digit in combination)
    )
