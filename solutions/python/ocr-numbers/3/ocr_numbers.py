...
def convert(input_grid: list[str]) -> str:
    """
    Attempt to convert a grid of glyphs into numerical characters given these rules:
    ...
    :param list[str] input_grid: grid of glyphs
    :return str: string representation of numbers
    """
    if len(input_grid) % 4 > 0 or not input_grid:
        raise ValueError("Number of input lines is not a multiple of four")
    if any(len(row) % 3 > 0 or not row for row in input_grid):
