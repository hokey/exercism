...
def score(hand_str: str) -> tuple[int,list[int]]:
    ...
    if is_straight and is_flush:
        return (8, values)
    if freq == [4, 1]:
        return (7, [value for value, suit in ordered])
    if freq == [3, 2]:
        return (6, [value for value, suit in ordered])
    if is_flush:
        return (5, values)
    if is_straight:
        return (4, values)
    if freq == [3, 1, 1]:
        return (3, [value for value, suit in ordered])
    if freq == [2, 2, 1]:
        return (2, [value for value, suit in ordered])
    if freq == [2, 1, 1, 1]:
        return (1, [value for value, suit in ordered])
...
def best_hands(hands: list[str]) -> list[str]:
    ...
    best = max(poker_score for poker_score, poker_hand in scored)
